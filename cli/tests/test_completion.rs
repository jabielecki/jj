...
fn test_files() {
    ...
    // another one on top of the conflicted, to test interdiff behavior
    create_commit(
        &test_env,
        &repo_path,
        "third",
        &["conflicted"],
        &[("f_added_3", Some("added_3\n"))],
    );
    ...
    @  kpqxywon test.user@example.com 2001-02-03 08:05:18 working_copy 63967bf8
    │  working_copy
    │  A f_added_2
    │  M f_modified
    ...
    │  M f_modified
    │  A f_renamed
    │ ×  yostqsxw test.user@example.com 2001-02-03 08:05:16 third 9b1e3427 conflict
    │ │  third
    │ │  A f_added_3
    ...
    let stdout = test_env.jj_cmd_success(&repo_path, &["--", "jj", "squash", "-f=root()+", "f_"]);
    insta::assert_snapshot!(stdout.replace('\\', "/"), @r"
    f_deleted	Added
    f_modified	Added
    f_not_yet_renamed	Added
